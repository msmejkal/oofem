...
#include "misesmattemperature.h"
#include "sm/Materials/isolinearelasticmaterialtemperature.h"
...
namespace oofem {
REGISTER_Material( MisesMatTemperature );


MisesMatTemperature::MisesMatTemperature( int n, Domain *d ) :
    StructuralMaterial( n, d ),
    linearElasticMaterialTemperature( n, d )
{
}


void MisesMatTemperature::initializeFrom( InputRecord &ir )
{
    StructuralMaterial::initializeFrom( ir );
    linearElasticMaterialTemperature.initializeFrom( ir ); // takes care of elastic constants

    //G = linearElasticMaterialTemperature.giveShearModulus();
    //G = compute_G( GaussPoint * gp, TimeStep * tStep )
    //K = linearElasticMaterialTemperature.giveBulkModulus();


    hType = 0;
    IR_GIVE_OPTIONAL_FIELD( ir, hType, _IFT_MisesMatTemperature_htype ); //hardening type

    if ( hType == 0 ) {
        IR_GIVE_FIELD( ir, sig0, _IFT_MisesMatTemperature_sig0 ); // uniaxial yield stress
        H = 0.;
        IR_GIVE_OPTIONAL_FIELD( ir, H, _IFT_MisesMatTemperature_h ); // hardening modulus
    } else if ( hType == 1 ) { //user defined hardening function
        IR_GIVE_FIELD( ir, h_eps, _IFT_MisesMatTemperature_h_eps );
        IR_GIVE_FIELD( ir, h_function_eps, _IFT_MisesMatTemperature_h_function_eps );

        if ( h_eps.at( 1 ) != 0. ) {
            throw ValueInputException( ir, _IFT_MisesMatTemperature_h_eps, "The first entry in h_eps must be 0." );
        }

        if ( h_eps.giveSize() != h_function_eps.giveSize() ) {
            throw ValueInputException( ir, _IFT_MisesMatTemperature_h_function_eps, "the size of 'h_eps' and 'h(eps)' must be the same" );
        }
    } else {
        throw ValueInputException( ir, _IFT_MisesMatTemperature_htype, "Unknown htype. Should be either 0 or 1.\n" );
    }

    omega_crit = 0;
    IR_GIVE_OPTIONAL_FIELD( ir, omega_crit, _IFT_MisesMatTemperature_omega_crit ); // critical damage

    a = 0;
    IR_GIVE_OPTIONAL_FIELD( ir, a, _IFT_MisesMatTemperature_a ); // exponent in damage law

    yieldTol = 1.e-6;
    IR_GIVE_OPTIONAL_FIELD( ir, yieldTol, _IFT_MisesMatTemperature_yieldTol ); // tolerance in the yield condition
}

// creates a new material status  corresponding to this class
MaterialStatus *
MisesMatTemperature::CreateStatus( GaussPoint *gp ) const
{
    return new MisesMatTemperatureStatus( gp );
}

FloatArrayF<1>
MisesMatTemperature::giveRealStressVector_1d( const FloatArrayF<1> &totalStrain,
    GaussPoint *gp,
    TimeStep *tStep ) const
{
...
#if 1
    auto status = static_cast<MisesMatTemperatureStatus *>( this->giveStatus( gp ) );
    ...
    this->performPlasticityReturn( totalStrain, gp, tStep );
    ...
    /*double E = linearElasticMaterialTemperature.give( 'E', gp ), nu = linearElasticMaterialTemperature.give( 'n', gp );*/
    double E = linearElasticMaterialTemperature.eval_E(gp,tStep), nu = linearElasticMaterialTemperature.eval_nu(gp,tStep);

    auto strain = status->getTempPlasticStrain();
    /////////////////////////////////////
    auto elStrain = status->giveTempStrainVector();
    elStrain.subtract( strain );
    /////////////////////////////////////

    strain[0]   = totalStrain[0];
    //strain[1] -= nu / E * status->giveTempEffectiveStress()[0]; // Is this ok? 
    //strain[2] -= nu / E * status->giveTempEffectiveStress()[0]; // Is this ok? 
    strain[1] -= nu * elStrain[0]; // Is this ok?
    strain[2] -= nu * elStrain[0]; // Is this ok? 

    status->giveStrainVector();
...
FloatArrayF<3>
MisesMatTemperature::giveRealStressVector_PlaneStress( const FloatArrayF<3> &totalStrain,
    GaussPoint *gp, TimeStep *tStep ) const
{
    auto status = static_cast<MisesMatTemperatureStatus *>( this->giveStatus( gp ) );
    // initialization
    const_cast<MisesMatTemperature *>( this )->initTempStatus( gp );
...
FloatArrayF<6>
MisesMatTemperature::giveRealStressVector_3d( const FloatArrayF<6> &strain, GaussPoint *gp,
    TimeStep *tStep ) const
{
    auto status = static_cast<MisesMatTemperatureStatus *>( this->giveStatus( gp ) );
    // subtract stress independent part
    ...
    //this->performPlasticityReturn( strainR, gp, tStep );
    this->performPlasticityReturn( strain, gp, tStep );
    double omega = computeDamage( gp, tStep );
...
void MisesMatTemperature::performPlasticityReturn( const FloatArray &totalStrain, GaussPoint *gp, TimeStep *tStep ) const
{
    auto status = static_cast<MisesMatTemperatureStatus *>( this->giveStatus( gp ) );
    double kappa;
    FloatArray plStrain;
    ...
    ////
    double E = linearElasticMaterialTemperature.eval_E( gp, tStep );
    double temper = this->giveTemperature( gp, tStep );
    ////
    double dKappa = 0.;
    // === radial return algorithm ===
    if ( totalStrain.giveSize() == 1 ) {
        //double E = linearElasticMaterialTemperature.give( 'E', gp );
        ///*trial stress*/
        //fullStress.resize( 6 );
        //fullStress.at( 1 ) = E * ( totalStrain.at( 1 ) - plStrain.at( 1 ) );

        fullStress.resize( 6 ); // trial stress
        fullStress.at( 1 ) = linearElasticMaterialTemperature.giveRealStressVector_1d( totalStrain - plStrain, gp, tStep ).at( 1 );
        double E = linearElasticMaterialTemperature.eval_E( gp, tStep );
        ...
        FloatArray elStrain = totalStrain;
        elStrain.subtract( plStrain );
        FloatArray trialStress    = linearElasticMaterialTemperature.giveRealStressVector_3d( elStrain, gp, tStep );

        // Contribution of plastic strain is neglected in incremental formulation in IsotropicLinearElasticMaterialTemperature ::giveRealStressVector_3d
        // hence it needs to be added here
        FloatMatrix d( linearElasticMaterialTemperature.give3dMaterialStiffnessMatrix( TangentStiffness, gp, tStep ) );
        FloatArray PlasticStrainContribution;
        PlasticStrainContribution.beProductOf( d, plStrain );
        trialStress += PlasticStrainContribution;

        auto tmp = computeDeviatoricVolumetricSplit( trialStress );
        FloatArray trialStressDev = tmp.first;
        auto trialStressVol = tmp.second;

        double G = linearElasticMaterialTemperature.compute_G( gp, tStep );

        ///////////////////////////////////////
        double K                  = linearElasticMaterialTemperature.giveBulkModulus( gp, tStep );
        auto tmp2                  = computeDeviatoricVolumetricSplit( elStrain );
        auto elStrainDev          = tmp2.first;
        auto elStrainVol          = tmp2.second;
        FloatArray trialStressDevTest = applyDeviatoricElasticStiffness( elStrainDev, G );
        /**************************************************************/
        double trialStressVolTest = 3 * K * elStrainVol;
        /**************************************************************/
        FloatArray diff = trialStressDevTest;
        diff.subtract( trialStressDev );
        double diffNorm = diff.computeNorm();
        if ( diffNorm > 1e-6) {
            double x = 1;
        }
        ...
        if ( yieldValue > 0. ) {
            ...
            ////
            double sig0fun = computeYieldStressPrime( kappa );
            ////
...
void MisesMatTemperature::performPlasticityReturn_PlaneStress( const FloatArrayF<3> &totalStrain, GaussPoint *gp, TimeStep *tStep ) const
{
    double E = linearElasticMaterialTemperature.eval_E( gp, tStep );
    double nu = linearElasticMaterialTemperature.eval_nu( gp, tStep );
    double G = linearElasticMaterialTemperature.compute_G( gp, tStep );
    double K  = linearElasticMaterialTemperature.giveBulkModulus( gp, tStep );

    MisesMatTemperatureStatus *status = static_cast<MisesMatTemperatureStatus *>( this->giveStatus( gp ) );
    double kappa;
    ...
    FloatArray trialStress = linearElasticMaterialTemperature.giveRealStressVector_PlaneStress( elStrain, gp, tStep );

    // Contribution of plastic strain is neglected in incremental formulation in IsotropicLinearElasticMaterialTemperature::giveRealStressVector_PlaneStress
    // hence it needs to be added here
    FloatMatrix d( linearElasticMaterialTemperature.givePlaneStressStiffMtrx( TangentStiffness, gp, tStep ) );
    FloatArray PlasticStrainContribution;
    PlasticStrainContribution.beProductOf( d, redPlStrain );
    trialStress += PlasticStrainContribution;
...
double
MisesMatTemperature::checkYieldStress( double &dKappa, double kappa, GaussPoint *gp, TimeStep *tStep ) const
{
    ...
    if ( hType == 1 ) {
        ...
        OOFEM_ERROR( "MisesMatTemp: Should not check yield stress for htype = 0\n" );
    }
}

double
MisesMatTemperature::computeYieldStress( double kappa, GaussPoint *gp, TimeStep *tStep ) const
{
    double yieldStress = 0.;
    if ( hType == 0 ) {
...
double
MisesMatTemperature::computeYieldStressPrime( double kappa ) const
{
...
double
MisesMatTemperature::computeDamageParam( double tempKappa ) const
{
    if ( tempKappa > 0. ) {
...
double
MisesMatTemperature::computeDamageParamPrime( double tempKappa ) const
{
    if ( tempKappa >= 0. ) {
...
double
MisesMatTemperature::computeDamage( GaussPoint *gp, TimeStep *tStep ) const
{
    auto status      = static_cast<MisesMatTemperatureStatus *>( this->giveStatus( gp ) );
...
double MisesMatTemperature::computeCumPlastStrain( GaussPoint *gp, TimeStep *tStep ) const
{
    auto status = static_cast<MisesMatTemperatureStatus *>( this->giveStatus( gp ) );
...
FloatMatrixF<6, 6>
MisesMatTemperature::give3dMaterialStiffnessMatrix( MatResponseMode mode,
    GaussPoint *gp,
    TimeStep *tStep ) const
{
    // start from the elastic stiffness
    auto d = this->linearElasticMaterialTemperature.give3dMaterialStiffnessMatrix( mode, gp, tStep );
    if ( mode != TangentStiffness ) {
        return d;
    }

    double G = linearElasticMaterialTemperature.compute_G( gp, tStep ); // Compute shear modulus 
...
FloatMatrixF<3, 3>
MisesMatTemperature::givePlaneStressStiffMtrx( MatResponseMode mmode, GaussPoint *gp, TimeStep *tStep ) const
{
    auto status = static_cast<MisesMatTemperatureStatus *>( this->giveStatus( gp ) );
    // start from the elastic stiffness
    auto d = linearElasticMaterialTemperature.givePlaneStressStiffMtrx( mmode, gp, tStep );
    ...
    //double E   = linearElasticMaterialTemperature.give( 'E', gp );
    //double nu  = linearElasticMaterialTemperature.give( 'n', gp );
    double E   = linearElasticMaterialTemperature.eval_E( gp, tStep );
    double nu   = linearElasticMaterialTemperature.eval_nu( gp, tStep );
    double G   = linearElasticMaterialTemperature.compute_G( gp, tStep ); // Compute shear modulus 
...
FloatMatrixF<1, 1>
MisesMatTemperature::give1dStressStiffMtrx( MatResponseMode mode,
    GaussPoint *gp,
    TimeStep *tStep ) const
{
    MisesMatTemperatureStatus *status = static_cast<MisesMatTemperatureStatus *>( this->giveStatus( gp ) );
    ...
    double tempKappa = status->giveTempCumulativePlasticStrain();
    double omega     = status->giveTempDamage();
    auto elastic     = this->linearElasticMaterialTemperature.give1dStressStiffMtrx( mode, gp, tStep );
...
int MisesMatTemperature::giveIPValue( FloatArray &answer, GaussPoint *gp, InternalStateType type, TimeStep *tStep )
{
    MisesMatTemperatureStatus *status = static_cast<MisesMatTemperatureStatus *>( this->giveStatus( gp ) );
...
////////////////////////////////////////////
//double MisesMatTemperature::eval_G( GaussPoint *gp, TimeStep *tStep ) const
//{
//    return linearElasticMaterialTemperature.compute_G( gp, tStep );
//}
//
//double MisesMatTemperature::eval_K( GaussPoint *gp, TimeStep *tStep ) const
//{
//    return linearElasticMaterialTemperature.giveBulkModulus( gp, tStep );
//}

////////////////////////////////////////////
//=============================================================================

MisesMatTemperatureStatus::MisesMatTemperatureStatus( GaussPoint *g ) :
...
void MisesMatTemperatureStatus::printOutputAt( FILE *file, TimeStep *tStep ) const
{
...
void MisesMatTemperatureStatus::initTempStatus()
{
...
void MisesMatTemperatureStatus::updateYourself( TimeStep *tStep )
{
...
double
MisesMatTemperature::give( int aProperty, GaussPoint *gp, TimeStep *tStep ) const
{
    if ( aProperty == 's' ) {
...
double MisesMatTemperature::giveTemperature( GaussPoint *gp, TimeStep *tStep ) const
{
...
void MisesMatTemperatureStatus::saveContext( DataStream &stream, ContextMode mode )
{
...
void MisesMatTemperatureStatus::restoreContext( DataStream &stream, ContextMode mode )
{
